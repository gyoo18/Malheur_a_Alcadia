...
def surModificationFenêtre(event):
    ...
    if jeu.vieille_taille != Vec2(jeu.tkracine.winfo_width(),jeu.tkracine.winfo_height()):
        ...
        style.configure("TLabel",font=("Noto Sans CJK KR Black",taille_police),foreground="#4b2e83",background="#d7bd97")
        style.configure("TFrame",background="#d7bd97")
        style.configure("TButton",font = ("Old English Text MT",taille_police//2),foreground = "#4b2e83",background="#4e89e8",borderwidth=5)
...
def jeu():
    ...
    if not fenetre.initialisé:
        ...
        dialogue = Texte(fenetre.frame,background="#d7bd97",foreground="#4b2e83",relief="flat",wrap="word",highlightthickness=0,height=5)
